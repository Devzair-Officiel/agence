...
- [x] Créer `test/unit/config/legal.spec.ts` — 15 cas (SIREN, SIRET, cohérence, RCS, NAF, TVA, hébergeur, identité éditeur validée).
- [x] Mettre à jour `test/unit/layout/SiteFooter.spec.ts` — tests explicites sur les 2 liens légaux.
- [x] Créer `test/e2e/legal-pages.spec.ts` — SSR, meta robots noindex/follow, sitemap exclusion, footer, contenu LCEN/RGPD, identité éditeur, Axe WCAG 2.2 AA.

**DEV-LEGAL-1 levé (2026-09-15) :** `publisherName`, `publisherAddress`, `publisherPhone`, `publicationDirector` renseignés avec les valeurs validées par le responsable légal. HTML SSR vérifié. Conformité LCEN art. 6-III-1 atteinte.
