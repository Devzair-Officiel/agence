...
// E2E — /services/design-ui-ux-identite-visuelle (SEO-COM-5B)
//
// Vérifie dans le HTML SSR et après rendu :
//   - HTTP 200 ;
//   - H1 unique présent en SSR ;
//   - title et meta description avec "design" / "identité" ;
//   - Service JSON-LD présent en SSR ;
//   - BreadcrumbList JSON-LD (Accueil → Services → Design) ;
//   - lien vers /realisations/kitchen-meat (maillage) ;
//   - lien vers /estimer-mon-projet et /contact ;
//   - Axe WCAG 2.2 AA ;
//   - responsive sans débordement horizontal.

const PATH = "/services/design-ui-ux-identite-visuelle"
...
test.describe("/services/design-ui-ux-identite-visuelle — SSR et contenu", () => {
  ...
  test("expose un H1 unique en SSR", async ({ page, request }) => {
    ...
    expect(body).toContain("identité")
  ...
  test("expose title et meta description design en SSR", async ({ request }) => {
    const { body } = await fetchSSR(request, PATH)
    expect(body).toMatch(/<title>[^<]*[Dd]esign[^<]*<\/title>/i)
  ...
  test("expose Service JSON-LD en SSR", async ({ request }) => {
    ...
    expect(body).toContain("/services/design-ui-ux-identite-visuelle")
  })
  ...
  test("affiche le fil d'Ariane : Accueil → Services → Design", async ({ page }) => {
    await page.goto(PATH)
    ...
    await expect(breadcrumb.getByText(/[Dd]esign/i)).toBeVisible()
  })
})

test.describe("/services/design-ui-ux-identite-visuelle — maillage", () => {
  test("propose un lien vers /realisations/kitchen-meat", async ({ page }) => {
    await page.goto(PATH)
    await expect(
...
test.describe("/services/design-ui-ux-identite-visuelle — accessibilité et responsive", () => {
...
test.describe("/services/design-ui-ux-identite-visuelle — sitemap", () => {
