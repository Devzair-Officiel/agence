...
// E2E — /services/application-web-metier (SEO-COM-5A)
//
// Vérifie dans le HTML SSR et après rendu :
//   - HTTP 200 ;
//   - H1 unique présent en SSR ;
//   - title et meta description avec "application" ;
//   - Service JSON-LD présent en SSR ;
//   - BreadcrumbList JSON-LD (Accueil → Services → Application web métier) ;
//   - lien vers /realisations/mizan (maillage) ;
//   - lien vers /estimer-mon-projet et /contact ;
//   - aucun lien vers un service planned ;
//   - Axe WCAG 2.2 AA ;
//   - responsive sans débordement horizontal.

const PATH = "/services/application-web-metier"
...
test.describe("/services/application-web-metier — SSR et contenu", () => {
  ...
  test("expose title et meta description application en SSR", async ({ request }) => {
    const { body } = await fetchSSR(request, PATH)
    expect(body).toMatch(/<title>[^<]*[Aa]pplication[^<]*<\/title>/i)
  ...
  test("expose Service JSON-LD en SSR", async ({ request }) => {
    ...
    expect(body).toContain("/services/application-web-metier")
  })
  ...
  test("affiche le fil d'Ariane : Accueil → Services → Application web métier", async ({ page }) => {
    await page.goto(PATH)
    ...
    await expect(breadcrumb.getByText(/[Aa]pplication/i)).toBeVisible()
  })
})

test.describe("/services/application-web-metier — maillage", () => {
  test("propose un lien vers /realisations/mizan", async ({ page }) => {
    await page.goto(PATH)
    await expect(
...
test.describe("/services/application-web-metier — accessibilité et responsive", () => {
...
test.describe("/services/application-web-metier — sitemap", () => {
