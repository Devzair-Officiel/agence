...
// E2E — /services/site-e-commerce (SEO-COM-5A)
//
// Vérifie dans le HTML SSR et après rendu :
//   - HTTP 200 ;
//   - H1 unique présent en SSR ;
//   - title et meta description avec "e-commerce" ;
//   - Service JSON-LD présent en SSR ;
//   - BreadcrumbList JSON-LD (Accueil → Services → E-commerce) ;
//   - lien vers /realisations/nidemiel (maillage) ;
//   - lien vers /estimer-mon-projet et /contact ;
//   - aucun lien vers un service planned ;
//   - Axe WCAG 2.2 AA ;
//   - responsive sans débordement horizontal.

const PATH = "/services/site-e-commerce"
...
test.describe("/services/site-e-commerce — SSR et contenu", () => {
  ...
  test("expose un H1 unique en SSR", async ({ page, request }) => {
    ...
    expect(body).toContain("boutique")
  ...
  test("expose title et meta description e-commerce en SSR", async ({ request }) => {
    const { body } = await fetchSSR(request, PATH)
    expect(body).toMatch(/<title>[^<]*[Ee]-?commerce[^<]*<\/title>/i)
  ...
  test("expose Service JSON-LD en SSR", async ({ request }) => {
    ...
    expect(body).toContain("/services/site-e-commerce")
  })
  ...
  test("affiche le fil d'Ariane : Accueil → Services → E-commerce", async ({ page }) => {
    await page.goto(PATH)
    ...
    await expect(breadcrumb.getByText(/[Ee]-?commerce/i)).toBeVisible()
  })
})

test.describe("/services/site-e-commerce — maillage", () => {
...
test.describe("/services/site-e-commerce — accessibilité et responsive", () => {
...
test.describe("/services/site-e-commerce — sitemap", () => {
