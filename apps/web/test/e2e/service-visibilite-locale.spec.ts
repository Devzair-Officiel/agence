...
// E2E — /services/visibilite-locale (SEO-COM-5C)
//
// Vérifie dans le HTML SSR et après rendu :
//   - HTTP 200 ;
//   - H1 unique présent en SSR ;
//   - title et meta description avec "local" / "visibilité" ;
//   - Service JSON-LD présent en SSR ;
//   - BreadcrumbList JSON-LD (Accueil → Services → Visibilité locale) ;
//   - lien vers /services/seo-referencement-naturel (maillage) ;
//   - lien vers /estimer-mon-projet et /contact ;
//   - Axe WCAG 2.2 AA ;
//   - responsive sans débordement horizontal.

const PATH = "/services/visibilite-locale"
...
test.describe("/services/visibilite-locale — SSR et contenu", () => {
  ...
  test("expose un H1 unique en SSR", async ({ page, request }) => {
    ...
    expect(body).toMatch(/[Vv]isibilité|[Ll]ocale/i)
  ...
  test("expose title et meta description local/visibilité en SSR", async ({ request }) => {
    const { body } = await fetchSSR(request, PATH)
    expect(body).toMatch(/<title>[^<]*([Ll]ocal|[Vv]isibilité)[^<]*<\/title>/i)
  ...
  test("expose Service JSON-LD en SSR", async ({ request }) => {
    ...
    expect(body).toContain("/services/visibilite-locale")
  })
  ...
  test("affiche le fil d'Ariane : Accueil → Services → Visibilité locale", async ({ page }) => {
    await page.goto(PATH)
    ...
    await expect(breadcrumb.getByText(/[Vv]isibilité locale/i)).toBeVisible()
  })
})

test.describe("/services/visibilite-locale — maillage", () => {
  test("propose un lien vers /services/seo-referencement-naturel", async ({ page }) => {
    await page.goto(PATH)
    await expect(
      page.locator('a[href="/services/seo-referencement-naturel"]').first(),
    ).toBeVisible()
  })
...
test.describe("/services/visibilite-locale — accessibilité et responsive", () => {
...
test.describe("/services/visibilite-locale — sitemap", () => {
