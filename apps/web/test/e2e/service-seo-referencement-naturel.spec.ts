...
// E2E — /services/seo-referencement-naturel (SEO-COM-5C)
//
// Vérifie dans le HTML SSR et après rendu :
//   - HTTP 200 ;
//   - H1 unique présent en SSR ;
//   - title et meta description avec "SEO" / "référencement" ;
//   - Service JSON-LD présent en SSR ;
//   - BreadcrumbList JSON-LD (Accueil → Services → SEO) ;
//   - lien vers /services/visibilite-locale (maillage) ;
//   - lien vers /estimer-mon-projet et /contact ;
//   - Axe WCAG 2.2 AA ;
//   - responsive sans débordement horizontal.

const PATH = "/services/seo-referencement-naturel"
...
test.describe("/services/seo-referencement-naturel — SSR et contenu", () => {
  ...
  test("expose un H1 unique en SSR", async ({ page, request }) => {
    ...
    expect(body).toMatch(/[Ss][Ee][Oo]|[Rr]éférencement/i)
  ...
  test("expose title et meta description SEO/référencement en SSR", async ({ request }) => {
    const { body } = await fetchSSR(request, PATH)
    expect(body).toMatch(/<title>[^<]*([Ss][Ee][Oo]|[Rr]éférencement)[^<]*<\/title>/i)
  ...
  test("expose Service JSON-LD en SSR", async ({ request }) => {
    ...
    expect(body).toContain("/services/seo-referencement-naturel")
  })
  ...
  test("affiche le fil d'Ariane : Accueil → Services → SEO", async ({ page }) => {
    await page.goto(PATH)
    ...
    await expect(breadcrumb.getByText(/SEO/i)).toBeVisible()
  })
})

test.describe("/services/seo-referencement-naturel — maillage", () => {
  test("propose un lien vers /services/visibilite-locale", async ({ page }) => {
    await page.goto(PATH)
    await expect(
      page.locator('a[href="/services/visibilite-locale"]').first(),
    ).toBeVisible()
  })
...
test.describe("/services/seo-referencement-naturel — accessibilité et responsive", () => {
...
test.describe("/services/seo-referencement-naturel — sitemap", () => {
