...
| `NUXT_PUBLIC_API_BASE_URL`        | `/api`                  | Base des appels API front (formulaire de contact, ressources).        |
| `NUXT_PUBLIC_TURNSTILE_SITE_KEY`  | `''` (vide → mode dev)  | Site-key publique Cloudflare Turnstile. Vide en dev : le widget émet un token `dev-noop` accepté par `AlwaysAllowTurnstileVerifier` côté API. |
| `NUXT_PUBLIC_TURNSTILE_ENABLED`   | `false` (safe)          | Interrupteur front Turnstile — doit rester aligné avec `TURNSTILE_ENABLED` côté API (cf. [ADR-008](../../docs/adr/ADR-008-mailer-ovhcloud-turnstile-optionnel.md)). `false` : aucun script `challenges.cloudflare.com` n'est chargé, le widget émet `dev-noop`. `true` : script Cloudflare chargé, widget standard. Divergence des deux flags = rejet systématique en prod. |
...
Treize suites Playwright couvrent l'accueil `/`, les deux pages
institutionnelles `/agence` et `/expertises` (Phase 7A) et les cinq
pages d'expertise détaillées `/expertises/{slug}` (Phase 7B). La page
...
| `test/e2e/home-sections-secondary.spec.ts`    | Ordre éditorial des 8 sections (ajout de `home-case`, `home-process`, `home-trust`, `home-cta`), 3 H2 secondaires verbatim, section `#realisations` en variante honest-state sans lien ni bouton, `<ol>` de 6 étapes de méthode dans l'ordre `Découverte`…`Évolution`, `<ul>` de 5 promesses dans l'ordre attendu, CTA hero « Découvrir nos réalisations » qui scrolle vers `#realisations`, lien header « Réalisations » → `/#realisations`, présence SSR de 18 signatures éditoriales exactes, Axe WCAG 2.2 AA, `prefers-reduced-motion`, responsive 320 / 390 / 768 / 1024 / 1440 sans débordement horizontal. |
| `test/e2e/home-cta-final.spec.ts`             | Section finale `#contact` (Phase 6B) : SSR eyebrow / H2 / paragraphe verbatim, position last-child de `.home-page`, présence d'un vrai `<form>` avec `name`/`email`/`message`/`consent`/honeypot `website`, aucun mailto/tel/href="#" fictif, CTA hero + CTA header desktop + lien footer atteignent `/#contact`, ouverture du menu mobile 390 px → CTA → fermeture avec scroll restauré, pas de débordement horizontal aux breakpoints 320/390/768/1440, Axe WCAG 2.2 AA sur `#contact`, `prefers-reduced-motion` (H2 visible, opacity 1). |
| `test/e2e/contact-form.spec.ts`               | Formulaire de contact (Phases 6B + 6C) : présence SSR du `<form>` et de ses champs (name/email/message/consent + honeypot `website` tabindex=-1), happy path avec token Turnstile `dev-noop` + 200 accepted → bandeau succès avec `request_id`, HTTP 400 `validation_failed` → messages par champ + `aria-invalid=true`, honeypot rempli + 202 → bandeau succès (contrat côté UI inchangé), HTTP 429 `rate_limited` → bandeau global avec `Retry-After` affiché, HTTP 503 `temporary_error` (Phase 6C) → bandeau verbatim « Service momentanément indisponible » + valeurs saisies préservées, Turnstile désactivé (défaut) → zéro requête vers `challenges.cloudflare.com` et zéro `<script src*="challenges.cloudflare.com">` (Phase 6C), validation client bloque une soumission vide sans appeler `fetch`, Axe WCAG 2.2 AA restreint au form, `prefers-reduced-motion` (champs focusables). Le backend est mocké via `page.route('**/api/contact', …)` : la suite ne dépend pas de la stack Symfony. |
| `test/e2e/home-structure.spec.ts`             | Contrôles structurels globaux sur `/` : rend sans erreur console ni warning vue-router, un seul H1, skip link cible un vrai `<main id="main-content">`, header et footer présents, aucune coordonnée fictive (`@example`, `lorem ipsum`, `mailto:`, `tel:`) tant que `site.contact` reste null. |
...
| `test/e2e/institutional-pages.spec.ts`        | Pages `/agence` et `/expertises` (Phase 7A, actualisée Phase 7B) : HTTP 200 sans erreur console, un unique H1 verbatim par page, eyebrow et introduction présents en SSR, SEO complet (title, description, canonical *ou* noindex, `og:url`), lien réel `/#contact` + lien réciproque `/agence` ↔ `/expertises`, cinq cartes `.expertise-overview-card` exposent chacune un lien vers `/expertises/{slug}` (Phase 7B), navigation principale expose `Agence`, `Expertises`, `Réalisations` + CTA `/#contact`, responsive 390 / 768 / 1440 sans débordement horizontal, Axe WCAG 2.2 AA sans violation `serious`/`critical`, `prefers-reduced-motion` (H1 visible). |
| `test/e2e/expertise-pages.spec.ts`            | Pages d'expertise détaillées `/expertises/{concevoir,construire,valoriser,visibilite,faire-evoluer}` (Phase 7B) : SSR 200 par slug, HTTP 404 explicite sur slug inconnu (jamais de fallback silencieux), sitemap XML contient les 5 nouvelles routes, JSON-LD `@type":"Service"` présent avec `provider: {"@id": "…/#organization"}` (référence l'Organization globale), fil d'Ariane accessible (`<nav aria-label="Fil d'Ariane">`, `aria-current="page"`), absence de contenu placeholder/lorem/TODO, 2 liens vers les pôles connexes, navigation mobile opérationnelle sur la page slug, responsive 320/390/768/1024/1440 sans débordement horizontal, Axe WCAG 2.2 AA sans violation `serious`/`critical`, `prefers-reduced-motion` (H1 visible). |
...
app/
  ...
  components/
    ...
    expertise/         Composants des pages d'expertise (ExpertiseOverviewCard, ExpertisePageHero, ExpertiseDeliverables, ExpertiseBenefits, ExpertiseRelatedPillars)
  ...
  pages/               Routes Nuxt (accueil `/`, `/agence`, `/expertises`, `/expertises/[slug].vue` dynamique — 5 slugs publiés en Phase 7B)
