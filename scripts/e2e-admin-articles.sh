#!/usr/bin/env bash
# Orchestrateur des fixtures E2E — Phase 8C3 (interface admin éditoriale).
#
# Rôle : purger les articles créés par la suite Playwright admin
# (`admin-editorial.spec.ts`). Contrairement à `e2e-fixtures.sh` (Phase 8B2)
# qui préchargeait un jeu de 10 articles, la suite 8C3 CRÉE les articles
# via l'interface admin elle-même — c'est précisément ce qu'on veut tester.
# Le script n'a donc qu'un rôle de nettoyage :
...
#
# Garde-fous (identiques à `e2e-fixtures.sh` — voir en-tête de ce script) :
#   - la constante `E2E_SLUG_PREFIX` est CODÉE EN DUR (`e2e-8c3-`) ;
#     aucune substitution possible via variable d'environnement ;
#   - `DELETE ... WHERE slug LIKE 'e2e-8c3-%'` — jamais `TRUNCATE`,
...
#
# Usage :
#   scripts/e2e-admin-articles.sh load    # purge préventive (état propre)
#   scripts/e2e-admin-articles.sh clear   # purge post-suite
#
...
readonly E2E_SLUG_PREFIX='e2e-8c3-'
...
case "$ACTION" in
  load)
    echo "État de départ propre : articles admin E2E purgés (préfixe '${E2E_SLUG_PREFIX}')."
    ;;
  clear)
    echo "Articles admin E2E purgés (préfixe '${E2E_SLUG_PREFIX}')."
    ;;
esac
